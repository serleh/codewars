...
// So we want each child to have the same amount of candies, only we can't exactly
// take any candy away from the kids, that would be even worse.
// Instead we decide to give each child extra candy until they all have the same amount.

// Task
// Your job is to find out how much candy each child has, and give them each additional candy until they
//  too have as much as the child(ren) with the most candy. You also want to
//   keep a total of how much candy you've handed out because reasons."
...
function candies(kids) {
  ...
  let max = Math.max.apply(null, kids);
  let extraCandies = kids.reduce((total, kid) => total + max - kid, 0);
  return extraCandies;
}
